...
describe('migration v20 — worker-subsystem table teardown (TD-265)', () => {
  ...
  describe('populated v19 DB carrying task + coordination rows', () => {
    ...
    it('advances schema_version to exactly 25', () => {
      migrateSchema(db);
    ...
    it('is idempotent — a second migrateSchema() does not throw and keeps version at 25', () => {
      migrateSchema(db);
      expect(getSchemaVersion(db)).toBe(25);
  ...
  describe('fresh v19 DB that never had the worker tables (idempotent drop)', () => {
    ...
    it('advances schema_version to exactly 25 with the no-op drop', () => {
      migrateSchema(db);
      expect(getSchemaVersion(db)).toBe(25);
      for (const t of WORKER_TABLES) {
        expect(tableExists(db, t)).toBe(false);
