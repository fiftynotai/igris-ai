...
export function buildJanitorSystemPrompt(): string {
  return [
    ...
    '  - merge    : the two are near-identical restatements. Provide a survivor_id',
    '               (the id that SURVIVES) and synthesized_content (the single best',
    '               merged statement, preserving every distinct detail from both).',
    ...
    '  - keep_both: NOT duplicates — leave both untouched (the safe default).',
    '',
    ...
    '  "synthesized_content": string, // merge ONLY: the merged statement',
