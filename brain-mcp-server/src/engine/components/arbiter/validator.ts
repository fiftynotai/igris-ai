 ...
 * `isArbiterResponseWellFormed` below): a MALFORMED / non-array response →
 ...
 * lenient parse `validateArbiterResponse` used, so the verdict matches what was
 * accepted. true → valid (possibly empty); false → malformed (→ parse_error).
 */
export function isArbiterResponseWellFormed(raw: string): boolean {
 ...
 * `[]` when the response is not a JSON array OR is a well-formed array whose
 * elements were all dropped — the engine tells these apart via
 * `isArbiterResponseWellFormed` (malformed → parse_error; well-formed empty →
 * success with zero candidates, TD-294). Never throws (the parseResponse contract).
 *
