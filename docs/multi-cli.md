...
"Add a New Harness (the four-material-surface runbook)" below — run it to walk
that same checklist step-by-step. (FR-179 retired the short-lived TD-224 "framework-dev"
...
## Orchestrator identity (the `os_identity` surface was retired — FR-202 M4)

The `os_identity` projection surface (TD-233) has been **removed**. Igris no
...
behaviour reaches antigravity via skills + the harness-specific delegation
recipe (the shared `~/.agents/skills` link + `core/os/harness-specific/antigravity.md`).
...
**But dynamic-agent harnesses still run faithful Igris agents at runtime (FR-183 → FR-202 M4).**
The N/A-static finding does NOT mean antigravity improvises its subagents. The
harness-agnostic **delegation recipe** lives in
`core/os/harness-specific/_delegation-recipe.md` and is referenced by each
`dynamic-define` harness's context-layer file (`core/os/harness-specific/<harness>.md`),
which the Boot stage loads for the Detect-resolved harness — gated on
`harnesses.<type>.delegation_model = dynamic-define`. FR-202 M4 re-homed the
recipe out of the (retired) identity region into this context layer; it stays out
of the `/hunt` skill so every delegating skill stays harness-agnostic. On a
dynamic-agent-only harness, each
...
same flatten → compile → drift scaffold as skills/agents/mcp):
...
- **opencode** hooks are covered by the FR-104 plugin (a `claude:merge` writes the
  ...
  config write).
- **antigravity** (FR-181) is a hook projection target: an `antigravity:merge`
  config-merges the hook GROUP into `~/.gemini/config/hooks.json` (gemini-cli hook
  format) → the antigravity bridge. codex (session_end-only) + gemini (gemini-cli
  0.45.0 has `gemini hooks` — onboarding tracked under FR-182) are NOT yet hook
  projection targets.
...
```json
{
  "cli_targets": {
    ...
    "gemini": {
      "hooks": {
        "events_covered": [],
        "note": "gemini-cli 0.45.0 has a `gemini hooks` subcommand; full onboarding tracked under FR-182. Not yet projected by Igris."
      }
    },
    "antigravity": {
      "hooks": {
...
> by teaching the four material surfaces how to project to it (and, if it defines
...
| Harness | Agent primitive | Skills method | MCP map key + entry shape | Hooks | Harness-specific file | Delegation model |
|---------|-----------------|---------------|---------------------------|-------|----------------------|--------------------|
...
- Identity: the `os_identity` projection surface was **retired** (FR-202 M4) —
  see "Orchestrator identity (the `os_identity` surface was retired)" above.
- Delegation model: the **harness-specific context layer** (FR-202 M4) —
  `harnesses.<type>.delegation_model` (`native-static` | `dynamic-define`) in
  `harness-manifest.json` is the applicability predicate. A `dynamic-define`
  harness has a `core/os/harness-specific/<harness>.md` file (pointing at the
  shared `_delegation-recipe.md`) that the Boot stage loads for the Detect-resolved
  harness, so skills delegate abstractly ("delegate to role X") and the model
  reads the per-harness *how* — zero per-skill branching. A native-static harness
  needs no file. A harness absent from the map defaults to `native-static`.
...
1. **Phase 0 — PROBE the loader first** (non-destructive, throwaway HOME): which
   ...
   (→ hard-link)? frontmatter/tools shape? MCP-permission key shape? does it load
   Igris agents statically (`native-static`) or only define subagents at runtime
   (`dynamic-define` — needs a harness-specific file)? The emit primitive, dir
...
10. **Delegation mechanism + harness-specific context layer (FR-202 M4)** — set
    ...
    via `subagent_type:<agent>`, and the harness needs no harness-specific file.
    `dynamic-define` (runtime subagent definition only): create
    `core/os/harness-specific/<NEW>.md` (frontmatter `harness: <NEW>` +
    `delegation_model: dynamic-define` + `summary:`; body points at the shared
    `core/os/harness-specific/_delegation-recipe.md`, never copy-pasting it), then
    re-run `core/scripts/gen_os_index.sh` so the harness-specific roster picks it
    up. The Boot stage loads ONLY the file whose `harness:` matches the
    Detect-resolved harness. Verify: the file appears in `core/os/INDEX.md`'s
    harness-specific roster, then live `<NEW> -p "what is your delegation
...
The doc above is the **why** (the harness abstraction + the four-material-surface
model + the delegation-mechanism context layer).
