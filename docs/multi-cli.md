...
Claude target paths are typically `~/.claude/skills` (per-block); the compiler
walks each `<skill>/SKILL.md` under `source` and emits one symlink per skill at
`<path>/<name>` → `<source>/<name>`. The skill-name resolution mirrors the
gemini converter's per-skill walk.
...
- `targets[].method` — valid `(type, method)` pairs: `codex/compiler` (codex:
  all non-Claude-only skills folded into one `AGENTS.md`, 32 KB cap),
  `gemini/converter` (gemini: one `{name}.toml` per skill), `claude/symlink`
  (claude: per-skill symlink at `<path>/<name>` → registry-vendored
  `<source>/<name>`). All three are first-class projection targets — the
  symlink IS the projection, anchored at the registry-vendored copy (FR-149).
  An invalid pair (e.g. `claude/compiler`) is rejected at schema validation.
...
1. **Manifest schema** — add the new type to the `targets[].type` enum in
   ...
   `["codex", "gemini", "claude"]`), add the chosen method to
   `surfaces.skills.targets[].method` (currently
   `["compiler", "converter", "symlink"]`), and add the new
   `(type, method)` pair to the `oneOf` constraint on the skills target
   item. Mirror the same pair in `valid_pairs` inside `_common.sh
   validate_manifest` and in `VALID_SKILL_TYPE_METHOD_PAIRS` inside
