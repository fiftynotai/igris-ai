...
literally: `files` also carries two FILE-CLASS exclusions that nothing restores —
`.d.ts.map` at any depth (TD-443) and `.js.map` under `dist/brain-mcp-server/`
only (TD-444, path-scoped by operator decision; cli's own maps still ship). A
...
TD-443 added the `.d.ts.map` file class and TD-444 the bundled brain's `.js.map`
(under `dist/brain-mcp-server/` only — cli's own maps still ship), both dropped
from the packlist and never restored, because a consumer could not resolve those
maps anyway.
...
that the packed size stays under the packed-size ceiling (introduced by FR-238, raised to +550 KB by TD-329, then RE-BASED and set to +150 KB by TD-374 on 2026-08-10 — a larger absolute cap, measured from a clean origin rather than an FR-238-era one — then RE-BASED again by TD-444 on 2026-09-06 after the vendored `.js.map` recovery, `1_863_420` → `1_843_276`, grant unchanged; the floor moved DOWN this time).
