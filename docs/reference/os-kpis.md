...
| pairing | a stop pairs with the latest open start of `(project, ceremony, this machine)` — a mirror of `agent_events.ts` `findOpenStart`; since BR-100 "this machine" is the machine identity (`machine_id` first — a stop written after a network change still closes its start, a stop carrying a foreign id never does — the `machine.aliases` list for rows whose `machine_id` is NULL, hostname-only on a brain older than instances v5); `duration_ms = CAST((julianday('now') − julianday(start.created_at)) * 86400000 AS INTEGER)` — a mirror of `DURATION_FROM_START_SQL`. Known limitation (the FR-267 class): two concurrent same-ceremony runs of one project on one host may mis-pair — counts stay right, durations may swap. |
