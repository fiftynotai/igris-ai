...
|---|---|---|---|
| claude | `PASS` on authentication: the allow, base and `--mcp-inventory` arms all answered (4.1 s / 3.7 s / 3.7 s), no MCP descendant, forward links intact, and the stored login is the account OAuth (no API key). The probe's verdict line reads `TOOL_SPAWNED`, but that is the TD-476 census flagging claude's own startup helpers: its hardened `git -c core.askPass= -c protocol.ext.allow=never …` repo probe, the `xcodebuild -license check` it triggers through the macOS git shim, and a short-lived shell. The claude child has no model tools (`--allowedTools ''`). TD-478 teaches the census those helpers | n/a (no refresh ran) | 2026-09-25, claude 2.1.281, this machine, run after the TD-471 watcher's `NO_CAPTURE` stop (30 h cap); `plans/td472-evidence/probe-claude-20260925T185832Z.jsonl` |
...
|---|---|---|---|
| claude | no MCP process spawned in any arm, including the `--mcp-inventory` arm (census `mcp_spawned` false), after the TD-471 watcher's `NO_CAPTURE` stop; this was the pre-deploy gate for the owned `.claude.json` | 2026-09-25, claude 2.1.281, this machine (`plans/td472-evidence/probe-claude-20260925T185832Z.jsonl`) | `--allowedTools ''` already denies every model tool (see the tool-posture table below). TD-478: the earlier `TOOL_SPAWNED` reading (`plans/td472-evidence/probe-claude-20260925T185832Z.jsonl`) was claude's OWN startup helpers, not a model tool — its hardened `git` repo probe and the `xcodebuild -license check` it triggers are now pinned in `HELPER_ARGS.claude` and classify `cli_helper`, and any unattributable exited, argv-less process (a `(sh)`/`(bash)`, or a second `(git)` probe sampled only after it ended) downgrades to `cli_helper` under `MODEL_HAS_NO_TOOLS` (claude only). The orchestrator re-runs the probe post-build to confirm it now reads `PASS`/`NO_TOOL_SPAWNED` |
...
- **No-model-tools exited-process downgrade (TD-478).** claude's `--allowedTools
  ''` means its model has NO tool to call at all. So an EXITED, argv-less
  descendant under it can only be claude's own helper: a `(sh)`/`(bash)` too
  short-lived to sample running, a second `(git)` probe sampled only after it
  ended (measured on an egress run), or an `(unzip)` (seen in the final claude
  PASS run). It is unmatchable against `HELPER_ARGS`, and
  it is downgraded to `cli_helper` rather than left as an
  unattributable `tool`. `MODEL_HAS_NO_TOOLS: Set<ExtractorHarness>` is seeded
  with `claude` only, per-harness like `HELPER_ARGS`. The negative control
  (`td472_census.test.ts` TC13) proves the scope: a LIVE, argv-bearing shell
  under the same flag still classifies `tool` — the downgrade fires ONLY on
  the unattributable exited case, never as a blanket per-harness exemption.
  TC14 proves the sibling: the same exited `(sh)` under opencode's existing
  `modelHasNoTools=false` construction still classifies `tool` — the rule is
  per-harness, not global.
