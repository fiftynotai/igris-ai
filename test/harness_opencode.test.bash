...
# (loadout-anchored symlink → harness.opencode.md; OpenCode's agent loader
...
# (compile + drift) with a synthesized vendored loadout, mirroring the
...
EOF
  ...
  local harness="$IGRIS_BRAIN_DIR/loadout/agents/ocdemo/harness.opencode.md"
  [ -f "$harness" ]
  ...
  [[ "$output" == *"body line"* ]]
  # Target is a symlink to the loadout-resident harness.opencode.md.
...
EOF
  ...
  run cat "$IGRIS_BRAIN_DIR/loadout/agents/octools/harness.opencode.md"
  [[ "$output" == *"tools:"* ]]
  [[ "$output" == *"  read: true"* ]]
  [[ "$output" == *"  grep: true"* ]]
...
# Seed a loadout skills tree + an opencode/command skills manifest, compile.
...
@test "compile: opencode/command → per-skill wrapper with generated-marker + @file directive" {
  ...
  # (not $HOME), so the @-target is the absolute loadout path.
  [[ "$output" == *"@$IGRIS_BRAIN_DIR/loadout/skills/alpha/SKILL.md"* ]]
  [[ "$output" == *'$ARGUMENTS'* ]]
...
@test "compile: opencode/command refuse-to-clobber a non-generated file at the wrapper path" {
  skip "FR-212d: opencode/command custom wrapper retired (skills now delegate to the skills CLI; placement covered by fr212-smoke)"
  mkdir -p "$IGRIS_BRAIN_DIR/loadout/skills/alpha" "$PROJ/.opencode/command"
  cat > "$IGRIS_BRAIN_DIR/loadout/skills/alpha/SKILL.md" <<'EOF'
---
name: alpha
...
{
  "version": 1, "agents": [],
  "surfaces": { "skills": { "source": "$IGRIS_BRAIN_DIR/loadout/skills", "layer": "personal",
...
EOF
  ...
  run cat "$IGRIS_BRAIN_DIR/loadout/agents/parity/harness.opencode.md"
  [ "$status" -eq 0 ]
