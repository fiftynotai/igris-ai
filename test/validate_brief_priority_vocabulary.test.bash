  ...
  if [ "$prio" = "NULL" ]; then
    sqlite3 "$FIXTURE_DB" \
      "INSERT INTO brief_status (project, brief_id, priority, title, status)
         VALUES ('$(sql_q "$proj")', '$bid', NULL, 'T $bid', 'Ready');"
  else
    sqlite3 "$FIXTURE_DB" \
      "INSERT INTO brief_status (project, brief_id, priority, title, status)
         VALUES ('$(sql_q "$proj")', '$bid', '$prio', 'T $bid', 'Ready');"
  fi
}
...
# V2 runs the real validator under /bin/bash; V2m the quoted-form mutant on a
...
@test "(V2) BR-104: PROJECT=it's-proj under /bin/bash -> the offender is NAMED (exit 1)" {
  init_fixture_db
  FIXTURE_PROJECT="it's-proj"
  seed_brief "FR-001" "P1-High"
  seed_brief "TD-300" "P4-Trivial"
  run_validator_bin_q
  [ "$status" -eq 1 ] || { echo "expected exit 1, got $status; output: [$output]"; return 1; }
  assert_output_has "P4-Trivial"
}

@test "(V2m) BR-104 mutant: the quoted-form validator under /bin/bash 3.x reads an EMPTY PASS on V2's world (exit 0)" {
...
@test "(V5-V2) BR-104 control: the same offender under PROJECT=repo fires on the real validator AND on the quoted-form mutant" {
  command -v python3 >/dev/null 2>&1 || skip "python3 not available"
  init_fixture_db
  seed_brief "FR-001" "P1-High"
  seed_brief "TD-300" "P4-Trivial"
