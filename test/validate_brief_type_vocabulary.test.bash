...
seed_brief() {
  ...
  if [ "$btype" = "NULL" ]; then
    ...
    sqlite3 "$FIXTURE_DB" \
      "INSERT INTO brief_status (project, brief_id, brief_type, title, status)
         VALUES ('$(sql_q "$proj")', '$bid', '$btype', 'T $bid', 'Ready');"
  fi
}
...
# V4 runs the real validator under /bin/bash; V4m the quoted-form mutant on a
...
@test "(V4) BR-104: PROJECT=it's-proj under /bin/bash -> the offender is NAMED (exit 1)" {
  init_fixture_db
...
@test "(V4m) BR-104 mutant: the quoted-form validator under /bin/bash 3.x reads an EMPTY PASS on V4's world (exit 0)" {
...
@test "(V5-V4) BR-104 control: the same offender under PROJECT=repo fires on the real validator AND on the quoted-form mutant" {
  command -v python3 >/dev/null 2>&1 || skip "python3 not available"
  init_fixture_db
  seed_brief "FR-001" "Feature"
  seed_brief "BR-050" "Frobnicate"
