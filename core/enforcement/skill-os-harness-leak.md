...
harness-specific tokens are by design or a declared single-harness capability:
`onboard-harness/SKILL.md` (the adapter-authoring guide),
`core/os/surfaces-detail.md` (documents the adapter boundary), and
`team/SKILL.md` (declared single-harness — Claude-native Agent-Teams, FR-202
M6; intentional, not a leak to remove). The former `igris_os.md → TD-247`
deferred-leak exemption was removed when FR-187 deleted the monolith (its
go-forward home is the scanned `core/os/` layer). The allowlist must never be
broadened to launder a real leak — a new entry is questioned in review (L-448).
