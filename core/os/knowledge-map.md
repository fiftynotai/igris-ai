...
| Catalog (reusable-assets catalog, the "lego" store) | reusable-asset references / layer-2 customizations — reference rows (what · where · when-to-use · how-to-integrate), not the asset code | brain DB `catalog` table (`knowledge.db`) | VPS push/pull (in SYNC_TABLES) |
